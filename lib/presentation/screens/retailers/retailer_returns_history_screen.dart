...
class RetailerReturnsHistoryScreen extends ConsumerStatefulWidget {
  const RetailerReturnsHistoryScreen({super.key});

  @override
  ConsumerState<RetailerReturnsHistoryScreen> createState() => _State();
}

class _State extends ConsumerState<RetailerReturnsHistoryScreen> {
  DateTime? _from;
  DateTime? _to;
            ...
            child: RefreshIndicator(
              ...
              child: state.isLoading
                  ...
                  : state.returns.isEmpty
                      ? _Empty(icon: Icons.assignment_return_outlined, label: 'No return entries found')
                      : ListView.builder(
                          padding: EdgeInsets.all(isDesktop ? 24 : 16),
                          itemCount: state.returns.length,
                          itemBuilder: (_, i) => _ReturnCard(
                              entry: state.returns[i], color: color, primary: primary),
                        ),
            ),
          ),
        ],
...
class _ReturnCard extends StatelessWidget {
  ...
  const _ReturnCard({required this.entry, required this.color, required this.primary});
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.assignment_return, color: color, size: 21),
                ),
          ...
          Padding(
            ...
            child: Wrap(
              ...
              children: entry.items.map((item) => _ProductChip(
                  ...
                  qty: item.quantity, color: color, isDark: isDark)).toList(),
            ),
          ),
        ],
      ),
...
// ── Shared helpers (copied into each file so they're self-contained) ──────────
...
class _ProductChip extends StatelessWidget {
  final String name; final int qty; final Color color; final bool isDark;
  ...
  Widget build(BuildContext context) => Container(
      ...
      child: Text('$name × $qty', style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w500,
