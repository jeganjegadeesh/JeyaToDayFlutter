...
class _SalesReportScreenState extends State<SalesReportScreen> {
  ...
  Future<void> _run() async {
    ...
    try {
      ...
      final result = await ReportService.sales(_type, from: from, to: to, retailerId: _retailerId);
  ...
  Widget build(BuildContext context) {
    ...
    final salesTypes = [
      {'value': 'total', 'label': t.t('totalSales')},
      {'value': 'holding', 'label': t.t('holdingSales')},
      {'value': 'retailer', 'label': t.t('retailerTotalSales')},
      {'value': 'holding_retailer', 'label': t.t('holdingRetailerTotalSales')},
    ];

    return Scaffold(
      appBar: AppBar(title: Text(t.t('salesReports'))),
      body: ListView(
        ...
        children: [
          ...
          if (_loading)
            ...
            _buildSalesResult(scheme, t),
        ],
      ),
    );
  }

  Widget _buildSalesResult(ColorScheme scheme, AppLocalizations t) {
    if (_result == null) return const SizedBox.shrink();
    switch (_type) {
      case 'total':
        return _totalSalesView(scheme, t);
      case 'holding':
        return _holdingSalesView(scheme, t);
      case 'retailer':
        return _retailerSalesView(scheme, t);
      case 'holding_retailer':
        return _holdingRetailerView(scheme, t);
      default:
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Rs. ${value.toStringAsFixed(2)}', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: c)),
          ],
        ),
      ),
    );
  ...
  /// Total Sales: Date/Retailer/Amount table for the selected window, with a
  /// last-month vs this-month comparison up top and a grand total at the bottom.
  Widget _totalSalesView(ColorScheme scheme, AppLocalizations t) {
    ...
    return Column(
      ...
      children: [
        ...
        if (rows.isEmpty)
          _emptyCard(scheme, t.t('noSalesInPeriod'))
        else
          _card(
            scheme,
            child: Column(
                ...
                SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    columns: [
                      ...
                      DataColumn(label: Text(t.t('amountCol'))),
                    ],
                    rows: rows.map((r) {
                      final date = DateTime.tryParse(r['date'] ?? '') ?? DateTime.now();
                      final amount = (num.tryParse('${r['amount'] ?? 0}') ?? 0).toDouble();
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text('Rs. ${periodTotal.toStringAsFixed(2)}',
  ...
  /// Holding Sales: list of retailers currently holding unbilled stock value.
  Widget _holdingSalesView(ColorScheme scheme, AppLocalizations t) {
    final rows = (_result!['rows'] as List).map((r) => Map<String, dynamic>.from(r)).toList();
    final total = (num.tryParse('${_result!['total_holding_value'] ?? 0}') ?? 0).toDouble();

    if (rows.isEmpty) return _emptyCard(scheme, t.t('noRetailerHoldingStock'));
    ...
    return _card(
      ...
      child: Column(
        children: [
          ...rows.map((r) {
            final value = (num.tryParse('${r['holding_value'] ?? 0}') ?? 0).toDouble();
            return ListTile(
              leading: const CircleAvatar(backgroundColor: Color(0x1A10B981), child: Icon(Icons.storefront_outlined, color: Color(0xFF10B981))),
              title: Text(r['retailer_name'] ?? '-', style: const TextStyle(fontWeight: FontWeight.w600)),
              trailing: Text('Rs. ${value.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w700)),
            );
          }),
          const Divider(height: 1),
          Padding(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(t.t('totalHoldingValue'), style: const TextStyle(fontWeight: FontWeight.bold)),
  ...
  /// Retailer Total Sales: a single retailer's own Date/Amount sales rows.
  Widget _retailerSalesView(ColorScheme scheme, AppLocalizations t) {
    final rows = (_result!['rows'] as List).map((r) => Map<String, dynamic>.from(r)).toList();
    final total = (num.tryParse('${_result!['total_sales'] ?? 0}') ?? 0).toDouble();
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        if (rows.isEmpty)
          _emptyCard(scheme, t.t('noSalesForRetailerPeriod'))
        else
          _card(
            scheme,
            child: Column(
              children: [
                ...rows.map((r) {
                  ...
                  return ListTile(
                    ...
                    trailing: Text('Rs. ${amount.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w700)),
                  );
                }),
                const Divider(height: 1),
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(t.t('totalSalesRow'), style: const TextStyle(fontWeight: FontWeight.bold)),
                      Text('Rs. ${total.toStringAsFixed(2)}',
                          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: _kAccentBlue)),
                    ],
                  ),
  ...
  /// Holding Retailer Total Sales: a single retailer's current holding,
  /// broken down by product (qty x rate = value).
  Widget _holdingRetailerView(ColorScheme scheme, AppLocalizations t) {
    final rows = (_result!['rows'] as List).map((r) => Map<String, dynamic>.from(r)).toList();
    final total = (num.tryParse('${_result!['total_holding_value'] ?? 0}') ?? 0).toDouble();
    final name = _result!['retailer_name'] ?? '-';
                ...
                SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    columns: [
                      ...
                      DataColumn(label: Text(t.t('rateCol')), numeric: true),
                      DataColumn(label: Text(t.t('value')), numeric: true),
                    ],
                    rows: rows.map((r) {
                      final qty = (num.tryParse('${r['qty'] ?? 0}') ?? 0).toDouble();
                      final rate = (num.tryParse('${r['rate'] ?? 0}') ?? 0).toDouble();
                      final value = (num.tryParse('${r['value'] ?? 0}') ?? 0).toDouble();
                      return DataRow(cells: [
